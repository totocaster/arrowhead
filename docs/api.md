...
- `arrowhead --mcp-server` — start the HTTP MCP transport (Axum-based JSON-RPC 2.0) with bearer/link-token authentication, IP allowlists, and health reporting. Pair it with a reverse proxy (nginx / Caddy / Cloudflare Tunnel) or a Tailscale Funnel when exposing it outside localhost. Key flags:
