...
| Proactive linking | In progress | Metric, note, and source context now share an internal evidence model that prefers explicit refs or source mentions, then structural day alignment, then inferred key-text matches; exact dotted metric-key mentions are now validated as true key tokens before becoming stronger evidence, note context now scopes same-day metric leads to explicitly named keys when present, source-qualified note evidence now prefers records from explicitly mentioned sources, note day pivots now prefer days backed by the strongest metric evidence, related-note pivots prefer stronger and fresher leads, nearby metric pivots now prefer same-source and same-family co-occurrences over arbitrary same-day metrics, inferred note tails are capped once stronger leads exist, and CLI/MCP payloads expose evidence kind plus inferred confidence on leads and pivots. Broader inferred-link confidence work is still pending. |
| Implementation | In progress | Metrics CRUD, rollups, note/context filters, full time-window and entity context, the first shared evidence-driven linking slice across note/metric/source, rollup-driven trend leads in metric/month/source/week/changed context, public evidence metadata on context leads/pivots, key/source-aware note↔metric evidence narrowing, evidence-aware note/day pivot ordering, and better nearby-metric ranking for metric context are now in; broader proactive linking and MCP discovery consolidation are still pending. |
...
- implement CLI CRUD
  ...
  - file-level `files create`, `files rename`, `files delete`, and `assign-missing-ids` are now wired
- implement MCP CRUD
  - record-level `create`, `update`, and `delete` are now wired
  - file-level `create_file`, `rename_file`, and `delete_file` are now wired
...
- add structural and inferred link generation
  ...
  - exact dotted metric-key references are now validated before they become stronger note evidence
  - note context now narrows same-day metric leads to explicitly named keys when notes mention both a day and metric keys
  - note context now prefers records from explicitly mentioned sources when notes mention both a source and metric key
  - note context now prefers metric-backed days and fresher related notes when choosing follow-up pivots
  - metric context now ranks nearby metric leads by same-source overlap, metric-family affinity, overlap frequency, and recency instead of raw same-day search order
