...
- Script Filter: `/usr/bin/python3 src/search.py` invokes `arrowhead search <mode> "<query>" --json --include-paths` (default limit 15) and maps the structured JSON payload (`results[*].id`, `relative_path`, `absolute_path`, `preview`, `reason`) into Alfred items. The CLI path is auto-discovered across common install locations (`~/.local/bin`, Homebrew, `/usr/local/bin`, etc.) before surfacing an error.
...
- Context: `mcp.context.get_day`, `mcp.context.get_week`, `mcp.context.get_month`, `mcp.context.get_changed`, `mcp.context.get_note`, `mcp.context.get_metric`, `mcp.context.get_source`
