...
When validating the fixtures with the CLI, `arrowhead search --format paths` emits absolute note locations for quick inspection, and semantic-only hits surface `"N/A"` in the BM25 column to indicate that no lexical score was used.
